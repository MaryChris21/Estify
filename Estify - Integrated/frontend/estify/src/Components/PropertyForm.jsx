...
import { ImageIcon, BadgePlus } from "lucide-react"
...
const PropertyForm = ({ isUpdate = false, originalPropertyId = null }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = isUpdate ? "http://localhost:5001/api/properties/update" : "http://localhost:5001/api/properties/post"

      const token = localStorage.getItem("agentToken")
      if (!token) {
        toast.error("Agent is not logged in. Please login to post a property.")
        return
      }
  ...
  return (
    <div
      className="min-h-screen w-full bg-cover bg-center relative flex items-center justify-center px-4 py-10"
    ...
    >
      ...
      <form
        ...
        className="relative z-10 space-y-5 bg-white p-8 rounded-2xl shadow-2xl max-w-2xl w-full border border-green-100"
      >
