...
const UserBookings = () => {
  ...
  const fetchUnavailableDates = async (propertyId, currentBookingId) => {
    try {
      const token = localStorage.getItem("token")
      const response = await axios.get(`http://localhost:5001/api/bookings/property/availability`, {
        headers: { Authorization: `Bearer ${token}` },
        params: { propertyId },
      })
  ...
  const isDateUnavailable = (date) => {
    return unavailableDates.some((range) => date >= range.start && date <= range.end)
  }
  ...
  const handleUpdateBooking = async (bookingId) => {
    ...
    const isOverlapping = unavailableDates.some((range) => {
      return editDates.startDate <= range.end && editDates.endDate >= range.start
    })
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-blue-50 to-gray-100">
        <div className="text-center">
          ...
          <p className="text-sm text-gray-500 mt-2">Please wait while we fetch your reservation details</p>
        </div>
      </div>
    )
      ...
      {showInquiryModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md">
            <div className="p-6">
              <h3 className="text-lg font-medium text-gray-900 mb-4">Submit Inquiry</h3>
              <textarea
        ...
        {bookings.length === 0 ? (
          <div className="bg-white rounded-xl shadow-lg overflow-hidden max-w-2xl mx-auto p-8 text-center">
            ...
            <p className="text-gray-500 mb-6">
              You haven't made any reservations. Start exploring properties to book your stay.
            </p>
            <button
          ...
          <div className="bg-white shadow-xl rounded-xl overflow-hidden">
            <div className="px-6 py-4 border-b border-gray-200 flex flex-wrap items-center justify-between">
              <div className="flex items-center space-x-4 mb-4 sm:mb-0">
                <h2 className="text-lg font-medium text-gray-900">Your Reservations</h2>
              ...
              <div className="flex space-x-2">
                ...
                <button
                  ...
                  className={`px-3 py-1 text-sm rounded-md ${
                    filter === "cancelled" ? "bg-gray-500 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"
                  }`}
                >
                  Cancelled
                </button>
...
<td className="px-6 py-5 whitespace-nowrap">
  <div className="text-sm font-medium text-gray-900 cursor-pointer hover:text-blue-600 transition duration-150"
       onClick={() => booking.property?._id && navigate(`/properties/${booking.property._id}`)}>
    {booking.property?.title || "Unknown Property"}
  </div>
  <div className="text-sm text-gray-500">
    {booking.property?.district || "N/A"}, {booking.property?.city || "N/A"}
  </div>
  <div className="text-xs text-gray-400 mt-1">Booked on {formatDate(booking.createdAt)}</div>
</td>
                      <td className="px-6 py-5 whitespace-nowrap">
                        {editingId === booking._id ? (
                          <DatePicker
                            ...
                            dayClassName={(date) => (isDateUnavailable(date) ? "booked-date" : null)}
                            excludeDateIntervals={unavailableDates.map((range) => ({
                              start: range.start,
                              end: range.end,
                            }))}
                          />
                      ...
                      <td className="px-6 py-5 whitespace-nowrap">
                        <div className="text-sm text-gray-900">
                          ...
                          {booking.price && booking.startDate && booking.endDate
                            ...
                            : "N/A"}
                          <div className="text-xs text-gray-500">Rs.{booking.price?.toFixed(2) || "N/A"} per night</div>
                        </div>
                      </td>
                      <td className="px-6 py-5 whitespace-nowrap">{getStatusBadge(booking.status)}</td>
