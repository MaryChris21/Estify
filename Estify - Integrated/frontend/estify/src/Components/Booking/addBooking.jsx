...
const AddBooking = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const propertyResponse = await axios.get(`http://localhost:5001/api/properties/${propertyId}`, config)
        ...
        try {
          const bookingsResponse = await axios.get(`http://localhost:5001/api/bookings/property/availability`, {
            ...config,
            params: {
              propertyId,
  ...
  const handleSubmit = async (e) => {
    ...
    const isOverlapping = bookedDates.some((booking) => {
      return formData.startDate <= booking.end && formData.endDate >= booking.start
    })
    ...
    try {
      ...
      const response = await axios.post("http://localhost:5001/api/bookings", bookingData, config)
      ...
      const errorMessage =
        ...
        (err.response?.status === 400
          ? "Property is not available during the requested dates"
  ...
  const dayClassName = (date) => {
    const isBooked = bookedDates.some((booking) => date >= booking.start && date <= booking.end)
  ...
  return (
    <div className="max-w-3xl mx-auto p-6 bg-white shadow-lg rounded-md">
      <h2 className="text-2xl font-semibold text-center mb-6">Book Property: {property.title}</h2>
      ...
      <div className="flex flex-col md:flex-row gap-6 mb-6">
        ...
        <div className="flex-1">
          <p>
            <strong>Type:</strong> For Rent
          </p>
          <p>
            <strong>Location:</strong> {property.district}
          </p>
          <p>
            <strong>Price:</strong> Rs.{property.price.toLocaleString()} per day
          </p>
          <p>
            <strong>Contact:</strong> {property.contactName} - {property.contactNumber}
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="form-group">
          ...
          {bookedDates.length > 0 && (
            <p className="text-sm text-gray-500 mt-1">Dates marked in red are already booked</p>
          )}
        </div>
